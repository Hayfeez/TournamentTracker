...
{
    public static class DeleteTournamentRound
    {
        ...
        public class Handler : IRequestHandler<Request, Result>
        {
            ...
            {
                var rounds = _readWriteContext.TournamentRounds.Where(x => x.AccountId == request.AccountId
                                                                                    && x.TournamentId == request.TournamentId).ToList();

                if (rounds.Count == 0)
                {
                    return new Result("This tournament has no rounds");
                }

                if (_readWriteContext.Fixtures.Any(x => x.AccountId == request.AccountId && x.TournamentId == request.TournamentId))
                {
                    return new Result("Tournament rounds cannot be deleted because they have fixtures already");
                }

              _readWriteContext.TournamentRounds.RemoveRange(rounds);
